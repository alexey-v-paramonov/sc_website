...
weight: 2
seo_title: "Авторизация API Интернет-радио | Радио-Точка API-ключи"
description: "Как авторизоваться в API платформы «Радио-Точка»: создание API-ключей в панели управления, передача токена в заголовке SC-API-KEY, примеры на Python и требования к HTTPS."
---
...
Для демонстрации создадим на сервере с ID 1 случайный плейлист с названием "Test Playlist". Запрос выполняется методом POST по URL **/api/v2/playlists/**:
